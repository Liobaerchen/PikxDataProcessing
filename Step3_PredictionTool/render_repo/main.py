"""
DISCLAIMER:
This is an edited version of prediction_tool_part_2.
It is minimally edited to be able to host the website without having to use commands
in the terminal, directly from GitHub.
HOWEVER: I would like to be graded on the other file. This is just to make the
demonstration easier -- ChatGPT edited this code for the render upload, so I
can't take credit.
"""
...
# make paths relative to this python file
BASE_DIR = os.path.dirname(__file__)

df_2_1 = pd.read_csv(os.path.join(BASE_DIR, "heatwaves_2_1.csv"))
df_5_6 = pd.read_csv(os.path.join(BASE_DIR, "heatwaves_5_6.csv"))
df_6_8 = pd.read_csv(os.path.join(BASE_DIR, "heatwaves_6_8.csv"))
...
df = pd.read_csv(os.path.join(BASE_DIR, "clean_livwell.csv"))
...
df = pd.read_csv(os.path.join(BASE_DIR, "worldbank_population.csv"), skiprows=4)
...
mapWithCoordinatesEtc = ColumnDataSource(data={
    'x': map_x,
    'y': map_y,
    'code': map_codes,
    'names': map_names,
    'color_val': [0]*len(map_codes),
    'size': [20]*len(map_codes)
})
...
line_src = ColumnDataSource(data={
    'year': years,
...
p_line = figure(
    title="Predicted DV prevalence in percent of women",
    sizing_mode="stretch_both"
)
...
sel = Select(
    ...
    value="Nigeria",
    options=sorted(abbrToCountry.values()),
...
def fillPlotsWithData():
    ...
    country_name = sel.value

    # get the current country name (from sel,
    # which is the dropdown where you can choose the country)

    country_abb = countryToAbbr[country_name]

    # convert to abbreviation to access the data

    y = sld.value

    # update the year based on the slider where the users can select a year
    ...
    heating_visual = (y - 2015) / (2100 - 2015)
    ...
    for i, code in enumerate(map_codes):
        ...
        base_temp = country_temp_norm.get(code, 0.5)

        # 0.5 is just gonna be the safety middle filler value
        ...
        bubble_size = (
            (initial_dv * 1.2)
            + ((predicted_dv_val - initial_dv) * 85)
        )
    ...
    total_women_country = int(
        (predicted_dv["High"][y][country_abb] / 100) * female_pop
    )

    total_women_country_low = int(
        (predicted_dv["Low"][y][country_abb] / 100) * female_pop
    )
    ...
    txt.text = (
        ...
        f"So, between the low and the high temperature increase scenario, "
        f"<b>~{difference_total_women:,}</b> more women experience domestic violence "
        f"in {y} in {country_name} than in 2015."
    )
...
layout = column(
    sel,
    row(p_line, p_map, sizing_mode="stretch_both"),
    sld,
    txt,
    sizing_mode="stretch_both"
)
